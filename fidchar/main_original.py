...
def generate_markdown_report(category_totals, yearly_amounts, yearly_counts, df, one_time, stopped_recurring, top_charities, charity_details, charity_descriptions, graph_info):
...
"""
    ...
    # Stopped recurring donations table
    stopped_data = []
    ...
    for i, (tax_id, _) in enumerate(top_charities.iterrows(), 1):
        ...
        if has_graph:
            report += f"| Charity Details | 10-Year Trend |\n"
            report += f"|:----------------|:-------------:|\n"
            report += f"| **Tax ID:** {tax_id}<br>**Sector:** {sector}<br>**Total Donated:** ${org_donations['Amount_Numeric'].sum():,.2f}<br>**Number of Donations:** {len(org_donations)} | ![10-year trend]({graph_filename}) |\n\n"
        else:
            # No graph - just show info in single column
            report += f"| Charity Details |\n"
            report += f"|:----------------|\n"
            report += f"| **Tax ID:** {tax_id}<br>**Sector:** {sector}<br>**Total Donated:** ${org_donations['Amount_Numeric'].sum():,.2f}<br>**Number of Donations:** {len(org_donations)}<br>*No donations in last 10 years* |\n\n"
        ...
        # Individual charity donation history table
...
def main():
    ...
    try:
        ...
        top_charities, charity_details, charity_descriptions, graph_info = analyze_top_charities(df, app_id=app_id, app_key=app_key)

        # Generate markdown report
        generate_markdown_report(category_totals, yearly_amounts, yearly_counts, df, one_time, stopped_recurring, top_charities, charity_details, charity_descriptions, graph_info)
